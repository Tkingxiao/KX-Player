...
export const TABLES = {
  ...
  bookmarks: 'bookmarks',
  renameBatches: 'rename_batches',
  renameItems: 'rename_items',
  rejectedSuggestions: 'rejected_suggestions',
  aiTranslateCache: 'ai_translate_cache',
  aiSubtitleContent: 'ai_subtitle_content',
  renameNameCache: 'rename_name_cache',
} as const
...
export const DB_COLUMNS = {
  ...
  rename_items: ['batch_id', 'old_path', 'new_path', 'level', 'applied'],

  rejected_suggestions: ['target_path', 'suggestion', 'rejected_at'],

  // 005_ai_cache.sql —— AI-15 的两层缓存。**没有 DTO 投影这两张表**（`ai_cache.rs` 只往外给
  // `bool` / `HashMap<String, String>` / 重算出来的 `translated` 标记），所以它们不进 `DTO_TABLES`；
  // 登记在这里是为了让 `check:columns` 把 DDL 与 `ai_cache.rs` 的 SQL 钉在一起：
  // 加列不登记、或 SQL 里写了个不存在的列名，都会红而不是「非领域表，跳过」。
  ai_translate_cache: [
    'kind',
    'source_path',
    'content_hash',
  ...
  ai_subtitle_content: ['content_hash', 'content', 'size', 'last_used'],

  rename_name_cache: ['cache_key', 'src', 'dst', 'notes', 'model', 'created_at'],
} as const
...
export const DTO_FIELDS = {
  ...
  /** `src/contracts/dto.ts` → `interface RenameBatch`（落在 rename_batches 表，`004_rename.sql`） */
  RenameBatch: {
    id: 'id',
...
export const DTO_NO_TABLE = {
  ...
  // —— 不进库：AI 线形状的请求/返回体（04 §5 的 7 张 AI 表已建 3 张，`004_rename.sql`；
  //    ai_providers / ai_jobs / ai_job_items / ai_cache 四张尚未建）——
  ...
  // —— 不进库：改名主干预览（`004_rename.sql` 已建表，但预览是**磁盘现状**的纯计算结果，不是批次账）——
  RenamePreview: 'rename_preview 的返回体：一层目录的档位分布 + 条目 + 截断/不可读标记，纯计算结果，不落库（批次账是 rename_batches）。',
  RenameCounts: 'RenamePreview 里的分布计数（rename::preview::Counts），由当前磁盘状态算出，不落库。',
  RenamePreviewItem: '一条改名预览（rename::preview::Item）：当前名 → 规范化后 + 档位/原因/冲突。只有 old/new 两列进得了 rename_items，档位·原因·冲突是判据不是账（落库时按 04 §7.7 写 applied=1）。',
  RenameReport: '一次改名执行的汇总（rename::report::Report）：计划/成功/跳过/失败明细 + history 落到了哪份文件。它是**这一次运行**的结果，不是库里的行（跑完之后的账在 rename_batches 的 note 里）。',
  RenameSuggestReport: '一次「生成译名」的汇总（rename::ai_ask::NameReport）：几条要问、几条命中缓存、几条没过校验。名字本身进 rename_name_cache，这份汇总只是给界面看的，不落库。',
} as const
