...
export const useTaxonomyStore = defineStore('taxonomy', () => {
  ...
  async function createCategory(parentId: number | null, name: string): Promise<void> {
    try {
      ...
      ui.toast(`已创建分类「${cat.name}」`, 'success')
      await refresh()
    } catch (e) {
  ...
  async function renameCategory(id: number, name: string): Promise<void> {
    try {
      await api.taxonomyRenameCategory(id, name)
      await refresh()
    } catch (e) {
      ui.toast(`分类重命名失败：${errText(e)}`, 'error')
    }
  }

  async function deleteCategory(id: number): Promise<void> {
    try {
      await api.taxonomyDeleteCategory(id)
    } catch (e) {
      ui.toast(`删除分类失败：${errText(e)}`, 'error')
      return
    }
  ...
  async function assignCategory(categoryId: number, trackIds: string[]): Promise<void> {
    try {
      const changed = await api.taxonomyAssignCategory(categoryId, trackIds)
      if (changed) {
        ui.toast(`已加入分类（${changed} 项）`, 'success')
        await refresh()
      }
    } catch (e) {
      ui.toast(errText(e), 'error')
  ...
  async function tagTracks(tagIds: number[], trackIds: string[], mode: 'add' | 'replace' = 'add'): Promise<void> {
    try {
      const changed = await api.taxonomyTagTracks(tagIds, trackIds, mode)
      if (changed) {
        ui.toast(`已打标签（${changed} 项）`, 'success')
        await refresh()
      }
    } catch (e) {
      ui.toast(errText(e), 'error')
  ...
  async function upsertTag(name: string, color?: string | null): Promise<Tag | null> {
    try {
      ...
      ui.toast(`新建标签失败：${errText(e)}`, 'error')
      return null
    }
  }
  ...
  async function renameTag(id: number, name: string): Promise<void> {
    try {
      await api.taxonomyRenameTag(id, name)
      ui.toast(`已重命名为「${name.trim()}」`, 'success')
      await refresh()
    } catch (e) {
      ui.toast(`标签重命名失败：${errText(e)}`, 'error')
    }
  }

  async function deleteTag(id: number): Promise<void> {
    try {
      await api.taxonomyDeleteTag(id)
    } catch (e) {
      ui.toast(`删除标签失败：${errText(e)}`, 'error')
      return
    }
    if (ui.tagIds.has(id)) {
  ...
  async function applySuggested(accepted: { trackId: string; tagName: string }[]): Promise<void> {
    ...
    try {
      const changed = await api.taxonomyApplySuggestedTags(accepted)
      ui.toast(changed ? `已写入 ${changed} 条标签` : '未写入标签（可能已存在）', changed ? 'success' : 'info')
