...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  getProgressAll: () => ipcRenderer.invoke('progress:getAll'),
  setProgress: (trackId: string, positionMs: number, completed: boolean | null, lastSpeed: number | null) => ipcRenderer.invoke('progress:set', trackId, positionMs, completed, lastSpeed),
  markTrackCompleted: (trackId: string, completed: boolean) => ipcRenderer.invoke('progress:markCompleted', trackId, completed),
  clearTrackProgress: (trackId: string) => ipcRenderer.invoke('progress:clear', trackId),
  listBookmarks: (trackId: string) => ipcRenderer.invoke('bookmarks:list', trackId),
  addBookmark: (trackId: string, atMs: number, label: string) => ipcRenderer.invoke('bookmarks:add', trackId, atMs, label),
  renameBookmark: (id: string, label: string) => ipcRenderer.invoke('bookmarks:rename', id, label),
  ...
  aiChat: (payload: { baseURL: string; apiKey: string; model: string; messages: { role: string; content: string }[]; temperature?: number }) =>
    ipcRenderer.invoke('ai:chat', payload),
  aiPing: (payload: { baseURL: string; apiKey: string; model: string }) => ipcRenderer.invoke('ai:ping', payload),
  aiListModels: (payload: { baseURL: string; apiKey: string }) => ipcRenderer.invoke('ai:listModels', payload),
  renameDir: (oldPath: string, newPath: string) => ipcRenderer.invoke('fs:renameDir', oldPath, newPath),
